...
func IsLiked(userID, postID int64) (bool, error) {
	...
	if _, err := database.DB.Exec("DEALLOCATE ALL"); err != nil {
		log.Printf("[IsLiked][WARN] Impossible de nettoyer les prepared statements: %v", err)
	}
...
func ToggleLike(userID, postID int64) (bool, error) {
	// Nettoyer les prepared statements existants
	if _, err := database.DB.Exec("DEALLOCATE ALL"); err != nil {
		log.Printf("[ToggleLike][WARN] Impossible de nettoyer les prepared statements: %v", err)
	}
...
func GetLikesCount(postID int64) (int, error) {
	// Nettoyer les prepared statements existants
	if _, err := database.DB.Exec("DEALLOCATE ALL"); err != nil {
		log.Printf("[GetLikesCount][WARN] Impossible de nettoyer les prepared statements: %v", err)
	}
...
func GetUserLikes(userID int64) ([]int64, error) {
	// Nettoyer les prepared statements existants
	if _, err := database.DB.Exec("DEALLOCATE ALL"); err != nil {
		log.Printf("[GetUserLikes][WARN] Impossible de nettoyer les prepared statements: %v", err)
	}

	query := "SELECT post_id FROM likes WHERE user_id=$1 ORDER BY created_at DESC"
	
	// Préparer la requête explicitement
	stmt, err := database.DB.Prepare(query)
	if err != nil {
		log.Printf("[GetUserLikes] Erreur préparation requête : %v", err)
		return nil, fmt.Errorf("erreur préparation requête GetUserLikes: %w", err)
	}
	defer stmt.Close()
